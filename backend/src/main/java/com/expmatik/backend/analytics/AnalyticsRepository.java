...
public interface AnalyticsRepository extends JpaRepository<Sale, UUID> {
    ...
    @Query("""
        ...
        AND (:startDate IS NULL OR b.invoice.invoiceDate >= :startDate)
        AND (:endDate IS NULL OR b.invoice.invoiceDate <= :endDate)
    ...
    @Query("""
        ...
        AND (:startDate IS NULL OR CAST(s.saleDate AS LocalDate) >= :startDate)
